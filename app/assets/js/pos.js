  ...
  $(document).ready(function () {
    ...
    function loadProducts() {
      $.get(api + "inventory/products", function (data) {
        data.forEach((item) => {
          var finalp = settings.rate * item.price;
          item.price2 = finalp;
        });
    ...
    $.fn.addProductToCart = function (data) {
      item = {
        ...
        sku: data.sku,
        price: settings.rate * data.price,
        quantity: 1,
      };
    ...
    $.fn.calculateCart = function () {
      ...
      $("#price").text(settings.symbol + total.toFixed(0));
      ...
      $("#gross_price").text(
        grossTotal.toLocaleString() + "  " + settings.symbol
      );
      ...
      $.each(cartList, function (index, data) {
        $("#cartTable > tbody").append(
          $("<tr>").append(
            ...
            $("<td>").append(
              $("<div>", { class: "input-group", id: "qtycart" }).append(
                $("<input>", {
                  restrictInput: true,
                  restrictInclude: "0123456789.",
                  type: "number",
                  width: "80px",
                  class: "form-control",
            ...
            $("<td>", {
              text:
                (data.price * parseFloat(data.quantity)).toLocaleString() +
                "  " +
                settings.symbol,
            }),
    ...
    $.fn.qtIncrement = function (i) {
      ...
      var integerPart = Math.floor(quantity);
      var fractionalPart = quantity - integerPart;
      ...
      if (product[0].stock == 1) {
        if (integerPart < product[0].quantity) {
          integerPart++;
          quantity = integerPart + fractionalPart;
    ...
    $.fn.qtInput = function (i) {
      var item = cart[i];
      var inputElem = $(this);
      inputElem.off("blur").on("blur", function () {
        item.quantity = parseFloat(inputElem.val());
        inputElem.renderTable(cart);
      });
    };
    // $.fn.qtInput = function (i) {
    //     item = cart[i];
    ...
    $.fn.submitDueOrder = function (status) {
      ...
      let paid =
        $("#payablePrice").val() == ""
          ? ""
          : parseFloat($("#payablePrice").val()).toFixed(0);
      let change =
        $("#change").text() == ""
          ? ""
          : parseFloat($("#change").text()).toFixed(0);
      ...
      if (paid != "") {
        payment = `<tr>
                    ...
                    <tr>
                        ...
                        <td>:</td>
                        <td>${
                          settings.symbol + Math.abs(change).toFixed(0)
                        }</td>
                    </tr>
                    <tr>
                        <td>Method</td>
        ...
        <table width="100%">
           ...
           ${items}              
            ...
            <tr>                        
                <td><b>Subtotal</b></td>
                <td>:</td>
                <td><h5><b>${settings.symbol}${subTotal.toFixed(
        0
      )}</b></h5></td>
            </tr>
            <tr>
                ...
                <td>${
                  discount > 0
                    ? settings.symbol + parseFloat(discount).toFixed(0)
                    : ""
                }</td>
            </tr>
            ...
            <tr>
                ...
                <td><h3>:</h3></td>
                <td>
                    <h2>${settings.symbol}  ${parseFloat(orderTotal).toFixed(
        0
      )}</h2>
                </td>
            </tr>
      ...
      printableContent.innerHTML = receipt;
      let defaultPrinterName = "";
      if (
        typeof navigator !== "undefined" &&
        typeof navigator.enumerateDevices === "function"
      ) {
        navigator.enumerateDevices().then(function (devices) {
          devices.forEach(function (device) {
            if (device.kind === "printer" && device.isDefault) {
              defaultPrinterName = device.label;
            }
          });
        });
      }
    ...
    $.fn.deleteOrder = function (index, type) {
      switch (type) {
        case 1:
          deleteId = allTransactions[index].order;
          break;
        case 2:
          deleteId = allTransactions[index].ref_number;
      }
    ...
    $.fn.editProduct = function (index) {
      ...
      $("#category option")
        ...
        .prop("selected", true);
      $("#sku").val(allProducts[index].sku);
      // split and trim barcode string to create an array
        ...
        if (result.value) {
          $.ajax({
            ...
            success: function (result) {
              ...
              Swal.fire("Done!", "Order deleted", "success");
            },
          });
        }
      });
    ...
    if (transactions.length > 0) {
      ...
      transactions.forEach((trans, index) => {
        ...
        transaction_list += `<tr>
                                ...
                                <td><button onClick="$(this).deletetransaction(${
                                  trans.order
        ...
        if (counter == transactions.length) {
          $("#total_sales #counter").text(
            parseFloat(sales).toLocaleString() + " " + settings.symbol
          );
...
function loadSoldProducts() {
  ...
  sold.forEach((item, index) => {
    ...
    integerPartTotal += Math.floor(qty);
    fractionalPartTotal += qty - Math.floor(qty);
    products++;

    let product = allProducts.find((selected) => selected._id === item.id);
    ...
    sold_list += `<tr>
            ...
            <td>${product.stock === 1 ? product.quantity : "N/A"}</td>
            <td>${
              (qty * parseFloat(item.price)).toLocaleString(undefined, {
  ...
  let items = integerPartTotal + Math.floor(fractionalPartTotal);
  let fractionalPart = fractionalPartTotal - Math.floor(fractionalPartTotal);

  if (fractionalPart > 0) {
    items += fractionalPart;
  }

  $("#total_items #counter").text(
    `${items.toLocaleString()}${settings.symbol}`
  );
  ...
  if (allTransactions[index].paid != "") {
    payment = `<tr>
                ...
                <tr>
                    ...
                    <td>${
                      settings.symbol +
                      Math.abs(allTransactions[index].change).toFixed(0)
                    }</td>
                </tr>
                <tr>
                    <td>Method</td>
    ...
    <table width="100%">
        ...
        <tr>
            ...
            <td>${
              discount > 0
                ? settings.symbol +
                  parseFloat(allTransactions[index].discount).toFixed(0)
                : ""
            }</td>
        </tr>
...
function authenticate() {
  $("#loading").append(
    `<div id="load"><form id="account"><div class="form-group">
    <p style="text-align:center;"><img id="theImg" src="assets/images/icon.ico" /></p>
    <input type="text" placeholder="Username" name="username" class="form-control"></div>
    ...
    <div class="form-group"><input class="btn btn-block btn-primary" value="Contact Us +96171465002"></div>
    </form>`
  );
}
